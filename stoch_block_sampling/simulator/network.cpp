...
void Network::GenerateStochBlock(size_t N, size_t N_C, double p_in, double p_out, std::mt19937 *rnd) {
  if( N%N_C != 0 ) {
    std::cerr << "[Warning] Network size " << N << " is not a multiple of C " << N_C << std::endl;
  }
  m_nodes.resize(N);
  for( size_t i=0; i < m_nodes.size(); i++ ) {
    m_nodes[i].m_id = i;
